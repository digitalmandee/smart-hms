...
-- Fix: source_id column is UUID, not text. Update the trigger function.
...
BEGIN
  ...
  LOOP
    ...
    IF NOT EXISTS (
      ...
      WHERE source_id = NEW.id AND doctor_id = v_doctor_id AND source_type = v_source_type
    ) THEN
      INSERT INTO public.doctor_earnings (
        ...
        v_doctor_id, NEW.organization_id, CURRENT_DATE, v_source_type, NEW.id,
