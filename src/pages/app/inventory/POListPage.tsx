...
export default function POListPage() {
  ...
  const filteredPOs = useMemo(() => {
    if (!purchaseOrders || !search) return purchaseOrders || [];
    const q = search.toLowerCase();
    return purchaseOrders.filter((po) =>
      po.po_number.toLowerCase().includes(q) ||
      (po.vendor?.name || "").toLowerCase().includes(q) ||
      (po.vendor?.vendor_code || "").toLowerCase().includes(q)
    );
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-3">
            <ListFilterBar search={search} onSearchChange={setSearch} searchPlaceholder="Search by PO number or vendor...">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">
                {statusFilter || search ? "Try a different filter" : "Create your first purchase order"}
              </p>
            ...
            <Table>
              ...
              <TableBody>
                {filteredPOs.map((po) => (
                  <TableRow key={po.id}>
                    <TableCell>
                      <Link to={`/app/inventory/purchase-orders/${po.id}`} className="font-medium text-primary hover:underline">
                        {po.po_number}
                      </Link>
                    </TableCell>
                    <TableCell>
