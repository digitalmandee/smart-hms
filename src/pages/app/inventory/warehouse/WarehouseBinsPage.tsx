...
export default function WarehouseBinsPage() {
  ...
  return (
    <div className="p-6">
      ...
      />
      <Card>
        <CardHeader><CardTitle>Bins {bins?.length ? `(${bins.length})` : ""}</CardTitle></CardHeader>
        <CardContent>
          {!storeId ? <p className="text-muted-foreground text-sm">Select a warehouse to view bins.</p> : isLoading ? <p className="text-muted-foreground text-sm">Loading...</p> : (
            <Table>
              ...
              <TableBody>
                ...
                {!bins?.length && <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground">No bins found</TableCell></TableRow>}
              </TableBody>
            </Table>
          )}
        </CardContent>
