...
import { Search, Plus, Users, Building2, Shield, Car } from "lucide-react";
...
export default function SupportStaffPage() {
  ...
  // Filter for support categories
  const supportCategoryIds = categories?.filter((c) =>
    c.name.toLowerCase().includes("support") ||
    c.name.toLowerCase().includes("admin") ||
    c.name.toLowerCase().includes("housekeeping") ||
    c.name.toLowerCase().includes("security") ||
    c.code?.toLowerCase().includes("sup") ||
    c.code?.toLowerCase().includes("adm")
  ).map((c) => c.id) || [];

  // Filter employees by support category or department
  const supportStaff = employees?.filter((emp) => {
    const dept = typeof emp.department === "object" ? emp.department?.name : "";
    const isSupportDept = dept?.toLowerCase().includes("admin") ||
      dept?.toLowerCase().includes("housekeeping") ||
      dept?.toLowerCase().includes("security") ||
      dept?.toLowerCase().includes("reception") ||
      dept?.toLowerCase().includes("maintenance");
    
    const desig = typeof emp.designation === "object" ? emp.designation?.name : emp.designation;
    const designation = desig?.toLowerCase() || "";
    const isSupportDesignation = designation.includes("driver") ||
      designation.includes("guard") ||
      designation.includes("cleaner") ||
      designation.includes("peon") ||
      designation.includes("sweeper");
    
    return supportCategoryIds.includes(emp.category_id || "") || isSupportDept || isSupportDesignation;
  }) || [];

  const filteredStaff = supportStaff.filter((emp) =>
  ...
  // Stats
  const totalStaff = supportStaff.length;
  const activeStaff = supportStaff.filter((e) => e.employment_status === "active").length;
  const securityStaff = supportStaff.filter((e) => 
    getDeptName(e).toLowerCase().includes("security") ||
    getDesignation(e).toLowerCase().includes("guard")
  ).length;
  const housekeepingStaff = supportStaff.filter((e) => 
    getDeptName(e).toLowerCase().includes("housekeeping") ||
    getDesignation(e).toLowerCase().includes("cleaner")
  ).length;
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-2xl font-bold">{securityStaff}</div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-2xl font-bold">{housekeepingStaff}</div>
                <p className="text-muted-foreground text-sm">Housekeeping</p>
              </div>
            </div>
          </CardContent>
        </Card>
