...
import { IpdDocNursing } from "@/components/ipd-docs/IpdDocNursing";
import { IpdDocCharges } from "@/components/ipd-docs/IpdDocCharges";
import { IpdDocDischarge } from "@/components/ipd-docs/IpdDocDischarge";
...
const pages = [
  ...
  { id: "nursing", label: "Nursing Station", component: IpdDocNursing },
  { id: "charges", label: "Charges", component: IpdDocCharges },
  { id: "discharge", label: "Discharge", component: IpdDocDischarge },
];

const IpdDocumentation = () => {
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    try {
      ...
      const blob=pdf.output("blob");const url=URL.createObjectURL(blob);const link=document.createElement("a");link.href=url;link.download="HealthOS24-IPD-Documentation.pdf";document.body.appendChild(link);link.click();
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      <div className={`print-container ${isPrintMode?"":"container mx-auto px-4 py-8"}`}>{isPrintMode?(<div ref={printContainerRef} className="space-y-0">{pages.map(p=>{const Comp=p.component;return<Comp key={p.id}/>})}</div>):(<div className="flex justify-center"><C/></div>)}</div>
    </div>
  );
};
export default IpdDocumentation;
