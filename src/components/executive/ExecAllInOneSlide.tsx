...
export function ExecAllInOneSlide() {
  return (
    <div className="slide flex flex-col items-center relative overflow-hidden bg-gradient-to-br from-primary/8 via-background to-primary/5">
      ...
      <div className="absolute top-6 right-6 text-sm text-muted-foreground font-medium bg-muted px-3 py-1 rounded-full">5 / 32</div>
      ...
      <div className="relative z-10 flex flex-col items-center w-full max-w-4xl pt-6">
        ...
        <p className="text-base text-muted-foreground mb-3 text-center">One platform. Every department. Fully integrated.</p>

        <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-muted/80 border mb-6">
        ...
        <div className="relative w-full flex items-center justify-center" style={{ minHeight: 340 }}>
            ...
            return (
              <div key={dept.name} className="absolute z-10" style={{ left: `calc(50% + ${x}px - 44px)`, top: `calc(50% + ${y}px - 44px)` }}>
                <div className="flex flex-col items-center w-[88px]">
                  <div className={`w-11 h-11 rounded-xl ${dept.color} flex items-center justify-center text-white shadow-lg`}>
          ...
          <svg className="absolute inset-0 w-full h-full z-0" style={{ minHeight: 340 }}>
            {departments.map((dept, i) => {
              const angle = (i * 360) / departments.length - 90;
              const rad = (angle * Math.PI) / 180;
              const radius = 150;
